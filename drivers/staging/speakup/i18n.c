...
char *spk_get(enum msg_index_t index)
{
	char *ch;
...
*/
ssize_t spk_set(enum msg_index_t index, char *text, size_t length)
{
	int rc = 0;
	char *newstr = NULL;
	unsigned long flags;
...
*/
struct msg_group_t *find_spk_group(const char *group_name)
{
